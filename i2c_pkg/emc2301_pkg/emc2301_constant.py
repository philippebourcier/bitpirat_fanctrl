# Address:
I2CBUS             = 1         # /dev/i2c-1
EMC2301_ADDRESS    = 0x2E   # 8 bit version
...
FAN_SETTING        = 0x40   # Displays Driver inputs or Direct control of fan
PWM_DIVIDE         = 0x41   # Store the divide ratio to set the frequency
FAN_CONF1          = 0x42   # FAN configuration #1
FAN_CONF2          = 0x43   # FAN configuration #2
GAIN               = 0x45   # Holds the gain terms
FAN_SPIN_UP        = 0x46   # Sets the config for Spin Up Routine
FAN_MAX_STEP       = 0x47   # Sets the maximum change per update
FAN_MIN_DRIVE      = 0x48   # Sets the minimum drive value
TACH_COUNT         = 0x49   # Holds the tachometer reading
FAN_FAIL_BAND      = 0x4A
FAN_FAIL_BAND_LB   = 0x4A   # Stores the number of Tach counts used to determine how the actual fan speed must match the target fan speed low byte
FAN_FAIL_BAND_HB   = 0x4B   # -- High byte
TACH_TARGET        = 0x4C
TACH_TARGET_LB     = 0x4C   # Holds the target tachometer low byte
TACH_TARGET_HB     = 0x4D   # -- High byte
TACH_READ          = 0x4E
TACH_READ_HB       = 0x4E   # Holds the tachometer reading high byte
TACH_READ_LB       = 0x4F   # -- Low byte
